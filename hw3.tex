...
\title{HW3 Problem Set}
\author{ECS 261}
\date{Due: Tuesday, February 24, 2026}
...
\item
It is sometimes possible that there are multiple different loop invariants
...
If it is possible, give an example and explanation;
...
\textbf{Grading notes:}
Please include a \emph{specific example program or programs} for each part, along with specific example precondition(s)/postcondition(s);
Please briefly explain why your example shows that the statement is possible,
or give a rigorous proof of why it is impossible.
...
The weakest precondition of \texttt{true} is \texttt{false}.
    ...
    % { weakest possible precondition here }
    % MyProgram;
    % { true }

    % weakest precondition of Q:

    %     { **weakest precondition goes here** }
    %     MyProgram
    %     { Q }

    % strongest postcondition of P:

    %     { P }
    %     MyProgram
    %     { **strongest postcondition goes here** }
...
Your code should pass the verifier (\texttt{dafny verify hw3.dfy}) without any errors
within a short time limit (under 10 seconds),
and it should not have any unproven axioms or assume statements (\texttt{dafny audit hw3.dfy}).
...
\begin{itemize}
\item Your code should be a file \texttt{hw3.dfy}, together with any necessary helper files.
